// WhatsApp message templates per status — opened via Linking.openURL
...
export function getReviewLinkUrl(phone: string, customerName: string, tokenNumber: string): string | null {
  const cleanPhone = phone.replace(/\D/g, "");
  if (!cleanPhone || cleanPhone === "0000000000") return null;
  const msg = `Hi ${customerName}! 🚲 Your bike service (${tokenNumber}) is complete. Please leave a review: https://bch.service/review/${tokenNumber}`;
  return `https://wa.me/91${cleanPhone}?text=${encodeURIComponent(msg)}`;
}
