...
/** A date N days before/after today (IST), as YYYY-MM-DD */
export function shiftTodayIST(days: number): string {
  const now = new Date(Date.now() + IST_OFFSET + days * 86400000);
  return now.toISOString().slice(0, 10);
}
